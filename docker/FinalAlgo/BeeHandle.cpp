...
void BeeHandle::movePoint(int P_idx, int A_idx) {
	...
	if (A_idx != -1) {
		float mag = pow(dist_x, 2) + pow(dist_y, 2);
		...
		//cout << "Point:     [" <<  points[P_idx].x << "," << points[P_idx].y << "]" << endl;
		//cout << "Attractor: [" <<  attractors[A_idx].x << "," << attractors[A_idx].y << "]" << endl;
	}
	...
	if (!(dist_x == 0 && dist_y == 0)) {
		...
		//if (A_idx != -1){
		//	cout << "New Point: [" <<  points[P_idx].x << "," << points[P_idx].y << "]" << endl;
		//}
	}
}
...
void BeeHandle::updatePoints() {
	...
	if (this->points.size() > this->attractors.size()) {
		...
		cout << "More Bees than Flowers" << endl;
	}
	else {
		candidates = this->attractors;
		picks = this->points;
		cout << "More Flowers than Bees" << endl;
	}
